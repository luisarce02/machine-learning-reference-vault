...
def load_and_prepare_data(file_path):
    ...
    print(f"Datos cargados: {data.shape[0]} filas y {data.shape[1]} columnas de características.")
...
def plot_predictions_vs_actual(y_actual, y_predicted, output_file="plot/mpg_predictions_vs_actual_manual.png"):
    print("\nGenerando gráfico de predicciones vs valores reales...")
    plt.figure(figsize=(8, 6))
    plt.scatter(y_actual, y_predicted, alpha=0.6, label="Predicciones")
    plt.plot([y_actual.min(), y_actual.max()], [y_actual.min(), y_actual.max()], color="red", linestyle="--", label="Línea ideal")
...
def train_and_evaluate_manual(file_path, learning_rate=0.01, iterations=1000):
    ...
    # Train the model
    theta, cost_history = gradient_descent(features, target, theta, learning_rate, iterations)
